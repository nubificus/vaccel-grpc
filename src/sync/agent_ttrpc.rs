...
pub trait VaccelAgent {
    ...
    fn update_session(&self, _ctx: &::ttrpc::TtrpcContext, _: super::session::UpdateSessionRequest) -> ::ttrpc::Result<super::agent::VaccelEmpty> {
...
pub fn create_vaccel_agent(service: Arc<Box<dyn VaccelAgent + Send + Sync>>) -> HashMap<String, Box<dyn ::ttrpc::MethodHandler + Send + Sync>> {
    ...
    methods.insert("/vaccel.VaccelAgent/UpdateSession".to_string(),
                    Box::new(UpdateSessionMethod{service: service.clone()}) as Box<dyn ::ttrpc::MethodHandler + Send + Sync>);
